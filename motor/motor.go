...
func (motor *Motor) EjecutarManifiesto(documento manifiesto.Manifiesto) (Resultado, error) {
	...
	if documento.Memoria != "" {
		...
		if err != nil { entidad.Estado = organismo.Error; return Resultado{}, err }
		entidad.Memoria = datos
	}

	for _, nombre := range documento.Capacidades { entidad.Capacidad[nombre] = true }
	...
	if decision == "ejecutar_gene" {
		...
		for _, nombreGene := range documento.Genes {
			...
			if !existe { entidad.Estado = organismo.Error; return Resultado{}, fmt.Errorf("Gene no registrado: %s", nombreGene) }
